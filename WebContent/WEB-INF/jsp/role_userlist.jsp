...
<body>
	...
	<div id="wrapper">
		...
		<div id="page-wrapper">
			<div class="row">
				<div class="col-lg-12">
					<h1 class="page-header">角色：${role_name}</h1>
				</div>
				<!-- /.col-lg-12 -->
			</div>
			<!-- /.row -->
			...
			<div class="row">
				<div class="col-lg-12">
					...
					<button class="btn btn-danger btn-xs" onclick="allcheck()">全选/取消</button>
					<form action="${pageContext.request.contextPath }/Role_deleteUserMany.action">
						<div class="panel-heading"><a href="${pageContext.request.contextPath }/Role_toaddUser.action?role_id=${role_id}" class="btn btn-danger btn-xs">${role_id}添加用户</a>											
						 <input class="btn btn-danger btn-xs" type= "submit" value = "批量删除"/>
						 </div>
						<!-- /.panel-heading -->
						<table class="table table-bordered table-striped">
							...
							<tbody>
								<c:forEach items="${page}" var="row">
									...
									<td><input type="checkbox" name="user_id" value="${row.user_id}"><br></td>
										 <input type="text" name="role_id" value="${row.user_roleid}" style="display: none;"> 
										<td>${row.user_id}</td>
										<td>${row.user_name}</td>
										<td>${row.user_password}</td>
										  <td>${row.role_name}</td>  
										<td>${row.user_state eq 1 ? '正常':'暂停'}</td>
										<%-- <td>${row.workflow_s eq 1 ? '是':'否'}</td> --%>
										 
										<td>
											<a href="${pageContext.request.contextPath }/findUserById.action?userid=${row.user_id}" class="btn btn-danger btn-xs">编辑</a>
											<a href="${pageContext.request.contextPath }/role_deleteuser.action?user_id=${row.user_id}&role_id=${row.user_roleid}" class="btn btn-danger btn-xs">${row.user_id}删除${row.user_roleid}</a>
										</td>
									</tr>
								</c:forEach>
						</tbody>
...
var checkAll = false;
function allcheck() {
    let inputs = document.getElementsByName('user_id');
    var checkInt = 0;
