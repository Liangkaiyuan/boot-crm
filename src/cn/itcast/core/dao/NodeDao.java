...
public interface NodeDao {
	...
	//查询流程名称对应的id
	String findeWorkflowIdbyName(@Param("workflowname")String workflowname);

	//查询节点信息根据id
	List<Node_new> findApprovalByid(@Param("workflow_id")String workflow_id);

	//更新节点信息
	void updateApproval(Node_new node_new);

	//根据流程名称查询流程id
	Integer findApprovalByName(@Param("workflow_name")String workflow_name);
	...
	//创建流程界面查询节点信息
	List<Node_approval> findNode_approveByid(@Param("workflow_id")String workflow_id);

	//未完成流程查询流程审批信息（精确到具体人员）
	List<Node_approval> Approval_showWorkflowdate(@Param("uid")String uid, @Param("workflow_id")String workflow_id);
